function fruitChatApp() {
    return {
        ...
        interestingFacts: [
            ...
            {category: 'history', text: 'Древние римляне использовали паутину как пластырь для ран.'},
        ...
        async callMistralAPI(message) {
            const API_KEY = this.apiKey || 'mdrxaCgD40KF6nLH172p9vN59EFJRnhP';
...
Твой характер: добрый, умный, терпеливый, с чувством юмора.`
        ...
        showRandomFact: function(category = null) {
            ...
            // Обновляем иконку фрукта в зависимости от категории факта
            ...
            if (factCategoryElement) {
                ...
                factCategoryElement.innerHTML = `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">${categoryNames[fact.category] || '📚 Факт'}</span>`;
            }
            ...
            if (window.appAnimations) {
                window.appAnimations.fadeInElement(factTextElement, 500);
            }
        ...
        async testApiKey() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.showStatus('Неверный API ключ', 'error');
                }
            } catch (error) {
                this.showStatus('Ошибка проверки ключа', 'error');
            }
        },
        ...
        saveApiKey() {
            ...
            this.showStatus('API ключ сохранён', 'success');
            this.closeModal();
        },
